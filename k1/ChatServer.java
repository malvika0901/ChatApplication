				...
				while (true) {
					...
					case Chat.CMD_UCAST:
						if (login == null) {
							...
							for(int i=3;i<textRecieved.length;i++)
							{
								mes.append(textRecieved[i]+" ");
					...
					case Chat.CMD_BCAST:
							if (login != null)
							{
								...
								for(int i=2;i<textRecieved.length;i++)
								{
									mesBroadCast.append(textRecieved[i]+" ");
									
								}
								mesBroadCast.substring(0, mesBroadCast.length()-1);
								String messageBroadCast = mesBroadCast.toString();
								String messageRec = messageBroadCast;
